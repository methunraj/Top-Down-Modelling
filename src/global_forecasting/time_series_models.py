...
class VARForecaster(BaseForecaster):
    ...
    def fit(self, data: pd.DataFrame) -> 'VARForecaster':
        ...
        # Check if we have additional variables beyond 'value'
        potential_vars = [col for col in data.columns if col not in ['date', 'value'] and data[col].dtype in ['int64', 'float64']]
        
        # If we have exogenous variables specified, use them
        if self.exog_vars is not None:
            available_exog = [col for col in self.exog_vars if col in data.columns]
            missing_exog = [col for col in self.exog_vars if col not in data.columns]
            
            if missing_exog:
                logger.warning(f"Exogenous variables not found in data: {missing_exog}")
            
            if available_exog:
                potential_vars.extend(available_exog)
        
        # If we still only have one variable, create derived variables for VAR
        if not potential_vars:
            logger.info("Only one variable available for VAR. Creating derived variables to enable multivariate modeling.")
            
            # Create lagged versions of the value column
            data_work = data.copy()
            data_work['value_lag1'] = data_work['value'].shift(1)
            data_work['value_lag2'] = data_work['value'].shift(2)
            
            # Create moving averages
            data_work['value_ma3'] = data_work['value'].rolling(window=3, min_periods=1).mean()
            data_work['value_ma5'] = data_work['value'].rolling(window=5, min_periods=1).mean()
            
            # Create growth rates (percentage change)
            data_work['value_pct_change'] = data_work['value'].pct_change().fillna(0)
            
            # Remove NaN values that might have been created by lagging
            ...
            # Set up endogenous variables with derived variables
            self.endog_cols = ['value', 'value_lag1', 'value_ma3']
            
        else:
            # We have additional variables, use them with 'value'
            self.endog_cols = ['value'] + potential_vars[:2]  # Limit to avoid overfitting
            logger.info(f"Using multivariate VAR with variables: {self.endog_cols}")
        
        # Ensure we have at least 2 variables for VAR
        if len(self.endog_cols) < 2:
            # Fallback: use a simple univariate model instead
            logger.warning("Insufficient variables for VAR model. Falling back to ARIMA.")
            self._setup_fallback_model(data)
            return self
        ...
        min_observations = max(10, 2 * len(self.endog_cols))
        if len(endog_data) < min_observations:
            logger.warning(f"Insufficient data points ({len(endog_data)}) for VAR model with {len(self.endog_cols)} variables. Need at least {min_observations}. Falling back to ARIMA.")
            self._setup_fallback_model(data)
            return self
        
        # Set up exogenous variables if provided and not already included
        self.exog_cols = None
        exog_data = None
        if self.exog_vars is not None:
            # Only use exog vars that aren't already in endog_cols
            remaining_exog = [col for col in self.exog_vars if col in data.columns and col not in self.endog_cols]
            if remaining_exog:
                self.exog_cols = remaining_exog
        ...
        try:
            ...
            if self.auto_order:
                # Limit max order based on data size
                max_order_adj = min(self.max_order, len(endog_data) // (2 * len(self.endog_cols)) - 1)
                max_order_adj = max(1, max_order_adj)
                
                order_results = select_order(endog_data, maxlags=max_order_adj, trend=self.trend)
            ...
            self.fitted_model = self.model.fit(maxlags=self.order, trend=self.trend, exog=exog_data)
            
            logger.info(f"Fitted VAR model with order {self.order} and {len(self.endog_cols)} variables")
            self.fitted = True
            
        except Exception as e:
            logger.error(f"Error fitting VAR model: {str(e)}")
            logger.warning("Falling back to ARIMA model.")
            self._setup_fallback_model(data)
            
        return self
    
    def _setup_fallback_model(self, data: pd.DataFrame) -> None:
        """
        Set up a fallback ARIMA model when VAR cannot be used.
        ...
        try:
            ...
            logger.warning("Using ARIMAForecaster as a fallback for VAR")
            
            # Create and fit an ARIMA model as fallback
            self.fallback_model = ARIMAForecaster(self.config)
            self.fallback_model.fit(data)
            self.fitted = True
            self.is_fallback = True
            logger.info("VAR fallback model fitted successfully")
        except Exception as e:
            logger.error(f"Failed to fit VAR fallback model: {e}")
            # If ARIMA also fails, try exponential smoothing
            try:
                from src.global_forecasting.statistical import ExponentialSmoothingForecaster
                
                self.fallback_model = ExponentialSmoothingForecaster(self.config)
                self.fallback_model.fit(data)
                self.fitted = True
                self.is_fallback = True
                logger.info("VAR fallback model (Exponential Smoothing) fitted successfully")
    ...
    def forecast(self, periods: int, frequency: str = 'Y') -> pd.DataFrame:
        ...
        try:
            ...
            if hasattr(self, 'is_fallback') and self.is_fallback and hasattr(self, 'fallback_model'):
                fallback_forecast = self.fallback_model.forecast(periods, frequency)
                self.forecast_result = fallback_forecast
                self.forecast_dates = forecast_dates
                self.confidence_intervals = getattr(self.fallback_model, 'confidence_intervals', None)
    ...
    def _predict_for_dates(self, dates: np.ndarray) -> np.ndarray:
        ...
        try:
            # Check if we're using the fallback model
            if hasattr(self, 'is_fallback') and self.is_fallback and hasattr(self, 'fallback_model'):
                fallback_forecast = self.fallback_model.forecast(max_period, 'M')
                
                # Map the forecasted values to the requested dates
                ...
                for i, period in enumerate(periods):
                    if period <= 0:
                        ...
                        predictions[i] = fallback_forecast['value'].iloc[period - 1]
...
class TemporalFusionTransformerForecaster(BaseForecaster):
    ...
    def _setup_mock_model(self, data: pd.DataFrame) -> None:
        ...
        # Store the history even if mock setup fails
        self.history = data.copy()
