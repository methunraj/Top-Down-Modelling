    ...
    def apply_indicator_adjustments(self, country_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'Year' not in adjusted_df.columns:
            ...
            for indicator_name, weight in self.indicator_weights.items():
                ...
                # Normalize indicator values to 0.5-1.5 range for adjustment factors
                # This creates a range where 1.0 is neutral (no adjustment)
                merged['normalized'] = (merged['Value'] - min_val) / range_val  # 0-1 range
                merged['normalized'] = merged['normalized'] * 1.0 + 0.5  # 0.5-1.5 range
