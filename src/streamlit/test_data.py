...
def generate_global_forecast() -> pd.DataFrame:
    ...
    # For test data, we'll simulate having historical data up to current year
    # and forecast for the next 6 years
    start_year = current_year - 6  # 6 years of historical data
    end_year = current_year + 6     # 6 years of forecast
    
    years = list(range(start_year, end_year + 1))
    historical_years = [y for y in years if y <= current_year]
    forecast_years = [y for y in years if y > current_year]
    
    # Create historical data with dynamic market size
    # Random starting market size between $100M and $1T
    start_value = np.random.uniform(1e8, 1e12)
...
def generate_country_historical() -> pd.DataFrame:
    ...
    # Define years dynamically based on global forecast data
    global_data = generate_global_forecast()
    historical_years = global_data[global_data['Type'] == 'Historical']['Year'].unique().tolist()
...
def generate_indicators(indicator_configs: Optional[List[Dict[str, Any]]] = None) -> Dict[str, pd.DataFrame]:
    ...
    # Define years dynamically based on the historical years from country data
    import datetime
    current_year = datetime.datetime.now().year
    # Use same logic as global forecast for consistency
    years = list(range(current_year - 6, current_year + 1))  # Last 6 years + current year
