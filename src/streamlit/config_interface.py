...
def render_market_distribution(config_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    tab_tiers, tab_growth, tab_smoothing, tab_distribution = st.tabs([
        "Tier Configuration", "Growth Constraints", "Smoothing", "Distribution Year"
    ])
    ...
    with tab_distribution:
        st.subheader("Distribution Start Year Settings")
        
        # Enable distribution from specific year
        enable_distribution_year = st.checkbox(
            "Start Distribution From Specific Year",
            value='distribution_start_year' in market_distribution,
            help="Preserve historical data exactly as-is before a specific year"
        )
        
        if enable_distribution_year:
            # Get default from existing setting or use a reasonable default
            current_year = int(market_distribution.get('distribution_start_year', 0))
            default_value = current_year if current_year > 0 else None
            ...
            # Add distribution year to settings
            market_distribution['distribution_start_year'] = distribution_year
        elif 'distribution_start_year' in market_distribution:
            # Remove distribution year if disabled
            del market_distribution['distribution_start_year']
