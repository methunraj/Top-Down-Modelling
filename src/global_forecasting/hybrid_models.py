...
class HybridETSARIMAForecaster(BaseForecaster):
    ...
    def fit(self, data: pd.DataFrame) -> 'HybridETSARIMAForecaster':
        ...
        try:
            ...
            # Get ARIMA fitted values - handle both auto_arima and SARIMAX results
            if self.arima_model is not None:
                # Debug logging
                logger.debug(f"ARIMA model type: {type(self.arima_model)}")
                logger.debug(f"ARIMA model attributes: {dir(self.arima_model)}")
                
                if hasattr(self.arima_model, 'fittedvalues'):
                    fitted_attr = getattr(self.arima_model, 'fittedvalues')
                    logger.debug(f"fittedvalues type: {type(fitted_attr)}")
                    logger.debug(f"Is callable: {callable(fitted_attr)}")
                    
                    # Check if fittedvalues is a method or property
                    if callable(fitted_attr):
                        self.arima_fitted = fitted_attr()
                    elif fitted_attr is not None:
                        self.arima_fitted = fitted_attr
                    else:
                        # Try predict method
                        if hasattr(self.arima_model, 'predict'):
                            try:
                                predict_result = self.arima_model.predict(start=0, end=len(self.residuals)-1)
                                self.arima_fitted = np.asarray(predict_result)
                            except Exception as e:
                                logger.debug(f"ARIMA predict failed: {e}")
                                self.arima_fitted = np.zeros(len(self.residuals))
                        else:
                            self.arima_fitted = np.zeros(len(self.residuals))
                elif hasattr(self.arima_model, 'predict'):
                    # For SARIMAX results, use predict method
                    try:
                        predict_result = self.arima_model.predict(start=0, end=len(self.residuals)-1)
                        self.arima_fitted = np.asarray(predict_result)
                    except Exception as e:
                        logger.debug(f"ARIMA predict failed: {e}")
                        # Fallback to zero residuals if prediction fails
                        self.arima_fitted = np.zeros(len(self.residuals))
                else:
                    self.arima_fitted = np.zeros(len(self.residuals))
            else:
            ...
            # Final debug check
            logger.debug(f"Final arima_fitted type: {type(self.arima_fitted)}")
            logger.debug(f"Is arima_fitted callable: {callable(self.arima_fitted)}")
            
            # Ensure arima_fitted is an array
            if callable(self.arima_fitted):
                logger.warning("arima_fitted is still callable after assignment, converting to zeros")
                self.arima_fitted = np.zeros(len(self.residuals))
            
            # Convert to numpy array if needed
            self.arima_fitted = np.asarray(self.arima_fitted)
    ...
    def forecast(self, periods: int, frequency: str = 'Y') -> pd.DataFrame:
        ...
        try:
            ...
            # Check the type of ARIMA model and use appropriate method
            try:
                # Check if it's a pmdarima ARIMA object
                if hasattr(self.arima_model, '__module__') and 'pmdarima' in self.arima_model.__module__:
                    # pmdarima ARIMA object - use predict method
                    arima_forecast = self.arima_model.predict(n_periods=periods)
                    logger.debug("Used pmdarima predict method")
                # Check if it's a statsmodels results object (from SARIMAX.fit())
                elif hasattr(self.arima_model, '__class__') and 'Results' in self.arima_model.__class__.__name__:
                    # statsmodels SARIMAXResults object - use forecast method
                    arima_forecast = self.arima_model.forecast(steps=periods)
                    logger.debug("Used statsmodels forecast method")
                else:
                    # Unknown model type - try various methods
                    if hasattr(self.arima_model, 'predict') and callable(self.arima_model.predict):
                        try:
                            # Try pmdarima style first
                            arima_forecast = self.arima_model.predict(n_periods=periods)
                        except:
                            # Try statsmodels style
                            arima_forecast = self.arima_model.predict(start=len(self.residuals), 
                                                                    end=len(self.residuals) + periods - 1)
                    else:
                        raise ValueError("Cannot determine how to forecast with this ARIMA model")
            except Exception as e:
                logger.warning(f"ARIMA forecast failed: {str(e)}, using zeros")
                arima_forecast = np.zeros(periods)
            
            # Step 3: Ensure forecasts are numpy arrays and combine
            ets_forecast = np.asarray(ets_forecast)
            arima_forecast = np.asarray(arima_forecast)
            
            # Ensure same length
            if len(ets_forecast) != len(arima_forecast):
                logger.warning(f"Forecast length mismatch: ETS={len(ets_forecast)}, ARIMA={len(arima_forecast)}")
                min_len = min(len(ets_forecast), len(arima_forecast))
                ets_forecast = ets_forecast[:min_len]
