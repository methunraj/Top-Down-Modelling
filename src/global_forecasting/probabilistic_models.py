    ...
    def fit(self, data: pd.DataFrame) -> 'BayesianStructuralTimeSeriesForecaster':
        ...
        if self.tfp_available:
            ...
            try:
                ...
                logger.error(f"Error during Stan model fitting: {str(e)}")
                self.fitted = False
                raise
                
        elif self.pystan_available:
            try:
                import pystan
        ...
        if self.tfp_available and hasattr(self, 'surrogate_posterior'):
            ...
            try:
                ...
                self.confidence_intervals = pd.DataFrame({
                    'date': future_dates,
                    'value': forecast_mean,
                    'lower': confidence_int.iloc[:, 0],
                    'upper': confidence_int.iloc[:, 1]
                })
