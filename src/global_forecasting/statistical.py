...
class CAGRForecaster(BaseForecaster):
    ...
    def fit(self, data: pd.DataFrame) -> 'CAGRForecaster':
        ...
        # Ensure years is not zero or too small
        if years <= 0 or years < 0.1:  # Less than ~36 days or zero/negative
            raise ValueError(f"Time period too short or invalid to calculate meaningful CAGR: {years} years")
        
        # Calculate CAGR with improved error handling
        if start_value > 0 and end_value > 0:
            try:
                # Additional safety check for years before division
                if years == 0:
                    raise ValueError("Cannot calculate CAGR with zero time period")
