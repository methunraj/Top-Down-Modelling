...
class MarketVisualizer:
    ...
    def _create_market_size_visualization(self, market_data: pd.DataFrame, 
        ...
        for p in ax.patches:
            ax.annotate(
                format_market_value(p.get_height()),
    ...
    def _create_growth_rates_visualization(self, market_data: pd.DataFrame, 
        ...
        # Calculate year-over-year growth rates (vectorized approach - first instance)
        # Sort data by country and year first
        market_data_sorted = market_data.sort_values(['idGeo', 'Country', 'Year'])
        
        # Calculate growth rate using vectorized operations
        market_data_sorted['Growth_Rate'] = (
            market_data_sorted.groupby(['idGeo', 'Country'])['Value']
            .pct_change() * 100
        )
        
        # Filter out first year for each country (NaN growth rates)
        growth_df = market_data_sorted.dropna(subset=['Growth_Rate'])
    ...
    def _add_growth_rates_sheet(self, workbook, writer, market_data: pd.DataFrame) -> None:
        ...
        Args:
            ...
            market_data: DataFrame with market forecast data
        """
        # Calculate year-over-year growth rates (vectorized approach - second instance)
        # Sort data by country and year first
        market_data_sorted = market_data.sort_values(['idGeo', 'Country', 'Year'])
