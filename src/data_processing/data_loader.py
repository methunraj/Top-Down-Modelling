...
class DataLoader:
    ...
    def load_global_forecast(self) -> pd.DataFrame:
        ...
        if 'Growth_Rate' not in df.columns:
            ...
            invalid_growth = df['Growth_Rate'].abs() > 1000  # More than 1000% growth
            if invalid_growth.any():
                invalid_count = invalid_growth.sum()
                logger.warning(f"Found {invalid_count} unrealistic growth rates (>1000%), capping at ±1000%")
                df.loc[df['Growth_Rate'] > 1000, 'Growth_Rate'] = 1000
                df.loc[df['Growth_Rate'] < -1000, 'Growth_Rate'] = -1000
    ...
    def _detect_wide_format(self, df: pd.DataFrame, id_col: str, country_col: str) -> bool:
        ...
        # Make year range configurable with reasonable defaults
        min_year = 1800  # Allow historical data back to 1800
        max_year = 2200  # Allow far future projections to 2200
        ...
        for col in df.columns:
            try:
                ...
                year = int(col)
                # Check if it looks like a year with flexible range
    ...
    def _wide_to_long_format(self, df: pd.DataFrame, id_col: str, country_col: str, 
        ...
        if len(df.columns) == 0:
            ...
            return pd.DataFrame(columns=['Year', 'Value'])
        # Identify year columns using same flexible range
        min_year = 1800  # Allow historical data back to 1800
        max_year = 2200  # Allow far future projections to 2200
