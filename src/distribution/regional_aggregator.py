...
from src.utils.math_utils import validate_data_consistency
    ...
    def aggregate_hierarchical(self, market_data: pd.DataFrame,
        ...
        for year in year_groups:
            ...
            for region in region_order:
                ...
                # Combine country and sub-region data safely
                data_to_combine = []
                if not country_data.empty:
                    data_to_combine.append(country_data)
                if sub_region_data:
                    # Filter out empty DataFrames
                    non_empty_sub_data = [df for df in sub_region_data if not df.empty]
                    data_to_combine.extend(non_empty_sub_data)
    ...
    def enforce_regional_consistency(self, market_data: pd.DataFrame,
        ...
        for year in year_groups:
            ...
            for region in region_order:
                ...
                # Use dynamic tolerance based on value magnitude
                if max(region_value, constituent_total) > 1000:
                    # For large values, use relative tolerance (0.01% = 0.0001)
                    tolerance = max(region_value, constituent_total) * 0.0001
                else:
                    # For small values, use absolute tolerance
                    tolerance = 0.01
