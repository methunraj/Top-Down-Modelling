...
def calculate_comprehensive_growth_metrics(data: pd.DataFrame, country_col: str) -> pd.DataFrame:
    ...
    # Ensure Year column is numeric before processing
    data = data.copy()
...
def render_growth_trajectory_analysis(growth_data: pd.DataFrame, country_col: str) -> None:
    ...
    for i, country in enumerate(selected_countries):
        country_df = growth_data[growth_data[country_col] == country]
        if country_df.empty:
        ...
        # Ensure years and values are properly paired and sorted
        if len(years) != len(values):
            logger.warning(f"Mismatched years and values for {country}")
            continue
            
        # Create a DataFrame to ensure proper sorting
        temp_df = pd.DataFrame({'Year': years, 'Value': values})
        temp_df = temp_df.sort_values('Year')
        
        sorted_years = temp_df['Year'].tolist()
        sorted_values = temp_df['Value'].tolist()
        
        # Normalize to base 100 for comparison
        if sorted_values[0] > 0:
            normalized_values = [(v / sorted_values[0]) * 100 for v in sorted_values]
        else:
...
def render_growth_forecasting(growth_data: pd.DataFrame, country_col: str) -> None:
    ...
    if selected_country:
        country_df = growth_data[growth_data[country_col] == selected_country]
        if country_df.empty:
            st.warning(f"No data found for {selected_country}")
            return
        country_row = country_df.iloc[0]
        ...
        # Get historical data and ensure it's sorted
        hist_years = country_row['Years']
        hist_values = country_row['Values']
        
        # Create DataFrame to ensure proper sorting
        hist_df = pd.DataFrame({'Year': hist_years, 'Value': hist_values})
        hist_df = hist_df.sort_values('Year')
        
        # Combine historical and forecast data into one continuous line
        all_years = hist_df['Year'].tolist()
        all_values = hist_df['Value'].tolist()
        
        # Add forecast data if available
        if forecast_result and 'values' in forecast_result:
            last_hist_year = all_years[-1]
            forecast_years_list = list(range(int(last_hist_year) + 1, 
                                           int(last_hist_year) + 1 + forecast_years))
            all_years.extend(forecast_years_list)
            all_values.extend(forecast_result['values'])
        ...
        if forecast_result and 'confidence_upper' in forecast_result and 'confidence_lower' in forecast_result:
            fig.add_trace(go.Scatter(
        ...
        if forecast_result:
            ...
            with col2:
                st.metric("Final Year Value", format_market_value(forecast_value))
