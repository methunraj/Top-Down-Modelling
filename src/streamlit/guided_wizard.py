...
class GuidedConfigurationWizard:
    ...
    def generate_quick_forecast(self):
        ...
        try:
            ...
            forecast_data = []
            # Generate dynamic market values based on relative scales
            # Start with normalized shares that sum to 1
            raw_shares = {country: np.random.uniform(0.5, 2.0) for country in countries}
            total_raw = sum(raw_shares.values())
            country_shares = {country: share/total_raw for country, share in raw_shares.items()}
            
            # Use a dynamic total market size
            total_market_size = np.random.uniform(1e8, 1e12)  # $100M to $1T range
            base_values = {country: total_market_size * share for country, share in country_shares.items()}
            ...
            for year in years:
                for i, country in enumerate(countries):
                    # Generate dynamic market values based on country rank
                    # Total market grows over time
                    total_market = np.random.uniform(5e8, 5e11) * (1.1 ** (year - 2024))
                    # Country shares based on rank (US largest, then decreasing)
                    country_shares = [0.35, 0.25, 0.15, 0.15, 0.10]  # Sum to 1.0
                    value = total_market * country_shares[i]
                    forecast_data.append({
                        'Country': country,
                        'Year': year,
                        'Value': value
