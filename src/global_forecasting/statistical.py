...
from src.utils.math_utils import calculate_cagr, calculate_growth_rate, apply_growth_bounds
...
class CAGRForecaster(BaseForecaster):
    ...
    def fit(self, data: pd.DataFrame) -> 'CAGRForecaster':
        ...
        # Calculate CAGR using safe calculation method
        calculated_cagr = calculate_cagr(start_value, end_value, years, handle_negative='geometric_mean')
        
        if calculated_cagr is not None:
            self.cagr = calculated_cagr
        else:
            # Fallback: try to use median growth rate from available data
            logger.warning(f"CAGR calculation failed for start={start_value}, end={end_value}")
            if len(period_data) > 2:
                period_data_sorted = period_data.sort_values('date')
                growth_rates = []
                
                for i in range(1, len(period_data_sorted)):
                    prev_val = period_data_sorted.iloc[i-1]['value']
                    curr_val = period_data_sorted.iloc[i]['value']
                    if prev_val != 0:  # Avoid division by zero
                        growth = calculate_growth_rate(curr_val, prev_val, annualize=True)
                        growth_rates.append(growth)
                
                if growth_rates:
                    self.cagr = np.median(growth_rates)
                    logger.info(f"Using median growth rate: {self.cagr:.2f}%")
                else:
                    self.cagr = 0
            else:
                self.cagr = 0
