...
def include_yaml_file(loader, node):
    ...
    if not _is_safe_path(include_path):
        raise ValueError(f"Unsafe include path detected: {include_path}")
    
    # Get the absolute path to the included file
    filename = os.path.join(loader._root, include_path)
    
    # Security: Ensure the resolved path is within the config directory
    config_dir = os.path.abspath(loader._root)
    resolved_path = os.path.abspath(filename)
    if not resolved_path.startswith(config_dir):
        raise ValueError(f"Include path outside config directory: {include_path}")
    
    # Check file size before loading (max 10MB)
    if os.path.exists(resolved_path):
        file_size = os.path.getsize(resolved_path)
        if file_size > 10 * 1024 * 1024:  # 10MB limit
            raise ValueError(f"Include file too large: {file_size} bytes")
    ...
    if not resolved_path.lower().endswith(('.yaml', '.yml')):
        raise ValueError(f"Only YAML files can be included: {include_path}")
...
def create_default_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "market_distribution": {
            ...
            "distribution_start_year": None,  # Set to a specific year to preserve historical data before this year and start fresh distribution from this year forward
            "manual_tiers": {
                "tier1": {
            ...
            "growth_constraints": {
                ...
                "manual_constraints": {
                    "max_growth_rate": 50,  # Realistic maximum growth of 50%
                    "min_growth_rate": -30,  # Maximum decline of 30%
                    "apply_scaling_by_market_size": True
                },
                "growth_bounds": {
                    "min": 0.7,  # 30% decline max (multiplier)
                    "max": 1.5   # 50% growth max (multiplier)
                }
            },
            "min_market_share": 0.001,  # Minimum 0.001% market share
            "zero_market_handling": "equal_distribution",
