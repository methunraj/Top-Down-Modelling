...
class IndicatorAnalyzer:
    ...
    def analyze_indicators(self) -> Dict[str, Dict[str, float]]:
        ...
        """
        logger.info("🔍 INDICATOR DEBUG: Starting analyze_indicators()")
        
        # Check what indicators are configured
        configured_indicators = self.config_manager.get_indicators()
        logger.info(f"🔍 INDICATOR DEBUG: Configured indicators: {configured_indicators}")
        logger.info(f"🔍 INDICATOR DEBUG: Number of configured indicators: {len(configured_indicators) if configured_indicators else 0}")
        
        # Load country historical data and indicators
        country_data = self.data_loader.load_country_historical()
        logger.info(f"🔍 INDICATOR DEBUG: Country data shape: {country_data.shape if country_data is not None else 'None'}")
        
        all_indicators = self.data_loader.load_all_indicators()
        logger.info(f"🔍 INDICATOR DEBUG: All indicators shape: {all_indicators.shape if all_indicators is not None else 'None'}")
        logger.info(f"🔍 INDICATOR DEBUG: All indicators columns: {list(all_indicators.columns) if all_indicators is not None and not all_indicators.empty else 'Empty or None'}")
    ...
    def apply_indicator_adjustments(self, country_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'Year' not in adjusted_df.columns:
            ...
            if 'market_share' in adjusted_df.columns:
                ...
                # Apply adjustment with bounds to prevent extreme values
                # Limit adjustment factor between 0.5 and 1.5 (50% decrease to 50% increase)
                bounded_score = np.clip(adjusted_df['score'], 0.5, 1.5)
                adjusted_df['market_share'] = adjusted_df['market_share'] * bounded_score
                
                # Use safe normalization method
                adjusted_df['market_share'] = normalize_to_sum(
                    adjusted_df['market_share'],
                    target_sum=100.0,
                    min_value=0.001
                )
        ...
        # If we have a Year column, process each year separately
            ...
            for year in adjusted_df['Year'].unique():
                ...
                for indicator_name, weight in self.indicator_weights.items():
                    ...
                    # Normalize indicator values to 0.5-1.5 range with safety checks
                    if range_val > 0:
                        merged['normalized'] = (merged['Value'] - min_val) / range_val
                        merged['normalized'] = merged['normalized'] * 1.0 + 0.5
                    else:
                        # If no range, all values are the same - use neutral value
                        merged['normalized'] = 1.0
                        logger.warning(f"No range in indicator {indicator_name} for year {year}, using neutral adjustment")
                    
                    # Apply weighted adjustment using additive method to prevent compounding
                    # Instead of multiplying factors, we add weighted deviations from neutral
                    adjustment = weight * (merged['normalized'] - 1.0)
                    merged['score'] = merged['score'] + adjustment
