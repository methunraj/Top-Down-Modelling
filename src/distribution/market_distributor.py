        ...
        for tier in range(best_n_clusters):
            ...
            if tier == 0:
                ...
                if not next_tier_data.empty:
                    ...
                    # Validate values before geometric mean calculation
                    if min_share > 0 and next_tier_max > 0:
                        # Use geometric mean to avoid unrealistic thresholds
                        threshold = np.sqrt(min_share * next_tier_max)
                    else:
                        # Fallback to arithmetic mean if geometric mean not feasible
                        logger.warning(f"Using arithmetic mean for tier {tier} threshold due to non-positive values")
                        threshold = (min_share + next_tier_max) / 2 if next_tier_max > 0 else min_share * 0.5
                else:
    ...
    def _project_market_shares(self, historical_shares: pd.DataFrame, forecast_years: List[int],
        ...
        for year in forecast_years:
            ...
            for tier in range(self.tiers if self.tiers else 3):
                ...
                # Calculate raw trend projection with validation
                # Ensure trend_factor is positive before applying fractional exponent
                safe_trend_factor = tier_df['trend_factor'].copy()
                
                # Handle negative trend factors
                negative_mask = safe_trend_factor < 0
                if negative_mask.any():
                    logger.warning(f"Tier {tier}: Found {negative_mask.sum()} negative trend factors, converting to absolute values")
                    safe_trend_factor = safe_trend_factor.abs()
                
                # Handle zero trend factors
                zero_mask = safe_trend_factor == 0
                if zero_mask.any():
                    logger.warning(f"Tier {tier}: Found {zero_mask.sum()} zero trend factors, setting to 1.0 (neutral)")
                    safe_trend_factor[zero_mask] = 1.0
                
                # Calculate raw trend projection safely
                exponent = diminishing_factor * years_ahead
                try:
                    raw_trend = safe_trend_factor ** exponent
                    # Validate result
                    if raw_trend.isna().any() or np.isinf(raw_trend).any():
                        logger.warning(f"Tier {tier}: Invalid trend calculation results, using neutral trend")
                        raw_trend = pd.Series([1.0] * len(raw_trend), index=raw_trend.index)
                except (ValueError, OverflowError) as e:
                    logger.warning(f"Tier {tier}: Error in trend calculation: {e}, using neutral trend")
                    raw_trend = pd.Series([1.0] * len(tier_df), index=tier_df.index)
            ...
            if share_deviation > 1.0:  # More than 1% deviation warrants investigation
                logger.warning(f"Year {year}: Market shares sum to {total_share:.2f}% (deviation: {share_deviation:.2f}%)")
            elif share_deviation > 5.0:  # More than 5% deviation is concerning
                logger.error(f"Year {year}: Large market share deviation detected: {share_deviation:.2f}%")
            ...
            if total_share > 0 and not np.isnan(total_share) and not np.isinf(total_share):
                ...
                if n_countries > 0:
                    equal_share = 100.0 / n_countries
                    year_projection['market_share'] = equal_share
                    logger.info(f"Year {year}: Applied equal distribution of {equal_share:.4f}% to {n_countries} countries")
                else:
