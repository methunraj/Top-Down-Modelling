...
class MarketDistributor:
    ...
    def distribute_market(self) -> pd.DataFrame:
        ...
        # Get distribution start year from configuration (if specified)
        # This preserves all historical data and only starts new distribution from this year forward
        distribution_start_year = self.distribution_settings.get('distribution_start_year', None)
        if distribution_start_year is not None:
            logger.info(f"Distribution start year: {distribution_start_year} - preserving historical data before this year")
        else:
            logger.info("No distribution start year set - processing all years normally")
        
        # Perform pre-flight validation of years
        self._validate_year_continuity(global_forecast, country_historical, global_year_col, distribution_start_year)
        ...
        # CRITICAL CHANGE: If distribution_start_year is set, we preserve all historical data before that year
        # and only apply new distribution logic from that year forward
        if distribution_start_year is not None:
            logger.info(f"Using distribution_start_year={distribution_start_year}. Preserving historical data before {distribution_start_year}. "
            ...
            # Path 1: Preserve historical data exactly as is (before distribution_start_year)
            preserved_historical = country_historical[country_historical['Year'] < distribution_start_year].copy()
            ...
            # Path 2: Process data from distribution_start_year onward  
            # CRITICAL FIX: Use ALL historical data for tier calculation and trends,
            # but only apply new distribution to years >= distribution_start_year
            logger.info(f"Using full historical data for tier calculation and trend analysis")
            
            # Calculate historical shares using ALL country historical data for better tier determination
            all_historical_years = sorted(country_historical['Year'].unique())
            historical_shares = self._calculate_historical_shares(
                country_historical, all_historical_years, id_col, country_col)
            
            # Get the latest year for tier determination (use latest available historical year)
            latest_historical_year = max(all_historical_years)
            ...
            # CRITICAL FIX: Get ALL years from global forecast >= distribution_start_year
            # Don't filter by Type here - we want both Historical and Forecast years that are >= distribution_start_year
            all_years_to_process = sorted(global_forecast[global_forecast[global_year_col] >= distribution_start_year][global_year_col].unique())
            logger.info(f"All years >= {distribution_start_year} in global forecast: {all_years_to_process}")
            ...
            historical_years_to_process = sorted(global_forecast[
                ...
                (global_forecast[global_year_col] >= distribution_start_year)
            ...
            logger.info(f"Historical years >= {distribution_start_year}: {historical_years_to_process}")
            logger.info(f"Forecast years >= {distribution_start_year}: {forecast_years_to_process}")
            ...
            # Project market shares for all years >= distribution_start_year (including historical years in that range)
            ...
            # CRITICAL FIX: Only keep historical shares for years < distribution_start_year
            # (years >= distribution_start_year will come from growth_constrained_shares)
            historical_shares_filtered = historical_shares[historical_shares['Year'] < distribution_start_year]
            logger.info(f"Filtered historical shares to years < {distribution_start_year}: {sorted(historical_shares_filtered['Year'].unique())}")
            ...
            # Calculate absolute market values using global forecast (only for years >= distribution_start_year)
            distributed_market = self._calculate_distributed_values(combined_shares, global_forecast, global_year_col, global_value_col, distribution_start_year)
            ...
            if not preserved_historical.empty:
                ...
                if not duplicates.empty:
                    ...
                    # Remove duplicates, keeping the preserved data for years < distribution_start_year
                    # and processed data for years >= distribution_start_year
                    before_count = len(final_result)
                    final_result = final_result.drop_duplicates(subset=[id_col, 'Year'], keep='last')
                    after_count = len(final_result)
            ...
            for year in sorted(final_result['Year'].unique()):
                ...
                if year not in global_values_final:
                    interpolated_value = self._interpolate_missing_year(year, global_values_final, distribution_start_year)
            ...
            logger.info("No distribution_start_year specified. Processing all years using standard distribution.")
            ...
            distributed_market = self._calculate_distributed_values(combined_shares, global_forecast, global_year_col, global_value_col, None)
            ...
            for year in sorted(smoothed_market['Year'].unique()):
                ...
                if year not in global_values_final:
                    ...
                    interpolated_value = self._interpolate_missing_year(year, global_values_final, None)
                    if interpolated_value is not None:
                        logger.info(f"Using interpolated global value for year {year}: {interpolated_value:.2f}")
                        global_values_final[year] = interpolated_value
                    else:
    ...
    def _apply_growth_constraints(self, projected_shares: pd.DataFrame, 
        ...
        if distribution_start_year is not None:
            # When distribution year is set, we need the year just before distribution
            # or the distribution year itself if it's in historical data
            hist_years = sorted(historical_shares['Year'].unique())
            
            if distribution_start_year in hist_years:
                reference_year = distribution_start_year
            else:
                # Find the latest year before distribution_start_year
                years_before = [y for y in hist_years if y < distribution_start_year]
                if years_before:
            ...
            logger.info(f"Using reference year {reference_year} for growth constraints (distribution year: {distribution_start_year})")
    ...
    def _calculate_distributed_values(self, combined_shares: pd.DataFrame,
                                     ...
                                     distribution_start_year: Optional[int] = None) -> pd.DataFrame:
        """
        ...
        for year in sorted(result_df['Year'].unique()):
            # Skip years before distribution_start_year if specified
            ...
            if year not in global_values:
                ...
                interpolated_value = self._interpolate_missing_year(year, global_values, distribution_start_year)
    ...
    def _validate_year_continuity(self, global_forecast: pd.DataFrame, country_historical: pd.DataFrame, 
                                  global_year_col: str, distribution_start_year: Optional[int] = None) -> None:
        """
        ...
        # If distribution year is set, validate continuity around it
        if distribution_start_year:
            # Check if distribution year exists in global forecast
            if distribution_start_year not in global_years:
                logger.warning(f"Distribution start year {distribution_start_year} not found in global forecast!")
                
            # Check years around distribution year
            years_around = [y for y in global_years if abs(y - distribution_start_year) <= 2]
            logger.info(f"Years around distribution year {distribution_start_year}: {years_around}")
            
            # Ensure we have at least one year before and after if possible
            years_before = [y for y in global_years if y < distribution_start_year]
            years_after = [y for y in global_years if y > distribution_start_year]
            
            if not years_before and distribution_start_year > min(global_years):
                logger.warning(f"No years found before distribution year {distribution_start_year}")
            if not years_after and distribution_start_year < max(global_years):
                logger.warning(f"No years found after distribution year {distribution_start_year}")
    ...
    def _interpolate_missing_year(self, year: int, global_values: Dict[int, float], 
                                  distribution_start_year: Optional[int] = None) -> Optional[float]:
        """
        ...
        # If distribution year is set and this is near it, be more careful
        if distribution_start_year and abs(year - distribution_start_year) <= 1:
            logger.info(f"Year {year} is near distribution year {distribution_start_year}, using careful interpolation")
