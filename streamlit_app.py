...
def render_home_page():
    ...
    if st.button("Load Test Data", key="load_test_data"):
        with st.spinner("Generating test data..."):
            ...
            st.success("Test data loaded successfully! You can now explore the application.")
...
def render_visualization_page():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Market Size", "Growth Analysis", "Market Share", "Advanced"])
    ...
    with tab1:
        ...
        with col1:
            ...
            view_type = st.radio("View Type", options=["Stacked", "Individual"], horizontal=True)
        
        with col2:
            # Years to include
            all_years = list(range(2020, 2028))  # Example years
            selected_years = st.multiselect("Select Years", options=all_years, default=all_years)
            
            # Scale type
            scale = st.radio("Scale", options=["Linear", "Logarithmic"], horizontal=True)
        
        # Chart display
        st.subheader("Market Size by Country")
        
        # In a real implementation, we would filter and visualize the actual data
        # For now, just a placeholder chart
        chart_data = pd.DataFrame(
            np.random.randn(len(selected_years), top_n),
    ...
    with tab2:
        ...
        with col1:
            # Growth metric
            growth_metric = st.radio("Growth Metric", options=["Year-over-Year", "CAGR"], horizontal=True)
            top_n = st.slider("Top N Countries", min_value=5, max_value=20, value=10, key="growth_top_n")
        
        with col2:
            # Period selection
            if growth_metric == "CAGR":
                start_year = st.selectbox("Start Year", options=list(range(2020, 2025)))
                end_year = st.selectbox("End Year", options=list(range(2021, 2028)))
            else:
                selected_years = st.multiselect("Select Years", options=list(range(2020, 2028)), 
                                              default=list(range(2020, 2023)), key="growth_years")
        
        # Chart display
        st.subheader("Growth Rates by Country")
        
        # In a real implementation, we would calculate and visualize actual growth rates
        # For now, just a placeholder chart
        if growth_metric == "CAGR":
            chart_data = pd.DataFrame(
                np.random.randn(1, top_n) * 10,  # Random CAGR values
                index=["CAGR"]
            )
            st.bar_chart(chart_data.T)
        else:
            chart_data = pd.DataFrame(
                np.random.randn(len(selected_years)-1, top_n) * 10,  # Random YoY growth values
                index=[f"{y}-{y+1}" for y in selected_years[:-1]]
            )
            st.bar_chart(chart_data)
    ...
    with tab3:
        ...
        with col1:
            # Reference year
            reference_year = st.selectbox("Reference Year", options=list(range(2020, 2028)))
            top_n = st.slider("Top N Countries", min_value=5, max_value=20, value=10, key="share_top_n")
        
        with col2:
            # Visualization type
            chart_type = st.radio("Chart Type", options=["Pie Chart", "Bar Chart", "Treemap"], horizontal=True)
            show_others = st.checkbox("Group Others", value=True)
        
        # Chart display
        st.subheader(f"Market Share ({reference_year})")
        
        # In a real implementation, we would calculate and visualize actual market shares
        # For now, just a placeholder text
        st.text("Market share visualization will be displayed here")
        
        # Temporary random data for visualization
        import random
        import plotly.express as px
        
        # Generate random market share data
        countries = ["United States", "China", "Japan", "Germany", "United Kingdom", 
                    "France", "India", "Brazil", "Canada", "South Korea"][:top_n]
        values = [random.uniform(5, 30) for _ in range(top_n)]
        ...
        if chart_type == "Pie Chart":
            fig = px.pie(share_data, values="Market Share (%)", names="Country", title=f"Market Share ({reference_year})")
            st.plotly_chart(fig)
        elif chart_type == "Bar Chart":
            fig = px.bar(share_data, x="Country", y="Market Share (%)", title=f"Market Share ({reference_year})")
            st.plotly_chart(fig)
        else:  # Treemap
            fig = px.treemap(share_data, path=["Country"], values="Market Share (%)", 
                            title=f"Market Share ({reference_year})")
            st.plotly_chart(fig)
    ...
    with tab4:
        ...
        # Create sub-tabs for different advanced visualizations
        subtab1, subtab2, subtab3 = st.tabs(["Regional Analysis", "Tier Analysis", "Correlation Analysis"])
        
        # Subtab 1: Regional Analysis
        with subtab1:
            st.subheader("Regional Market Analysis")
            st.text("Regional analysis visualizations will be displayed here")
            
            # In a real implementation, we would have a map visualization here
            
        # Subtab 2: Tier Analysis
        with subtab2:
            st.subheader("Market Tier Analysis")
            st.text("Tier-based analysis visualizations will be displayed here")
            
            # In a real implementation, we would show tier-based visualizations
            
        # Subtab 3: Correlation Analysis
        with subtab3:
            st.subheader("Indicator Correlation Analysis")
            
            if not st.session_state.indicators:
                st.warning("No indicators available for correlation analysis.")
            else:
                st.text("Indicator correlation visualizations will be displayed here")
                
                # In a real implementation, we would show correlation matrices and other visualizations
