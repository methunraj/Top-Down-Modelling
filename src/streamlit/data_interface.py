...
def render_data_upload(config_manager: ConfigurationManager, data_loader=None) -> Dict[str, Any]:
    """
            ...
            try:
                ...
                if is_valid:
                    ...
                    if st.button("Save Global Market Data"):
                        ...
                        # Also save to configured file path for persistence
                        try:
                            source_path = config_manager.get_data_source_path('global_forecast')
                            import os
                            ...
                            st.success(f"Global market data saved to session and file: {source_path}")
                            
                            # Clear cache so it reads the new file
                            if data_loader:
                                data_loader.clear_cache()
                        except Exception as e:
                            st.warning(f"Saved to session but couldn't save to file: {str(e)}")
                ...
                if is_valid:
                    ...
                    **Data Summary:**
                    ...
                    if st.button("Save Country Historical Data"):
                        # Ensure data has proper column names before saving
                        save_df = df.copy()
                        
                        # Get expected column names
                        try:
                            country_mapping = config_manager.get_column_mapping('country_historical')
                            expected_id_col = country_mapping.get('id_column', 'idGeo')
                            expected_country_col = country_mapping.get('country_column', 'Country')
                            
                            # Check if we need to map columns to standard names
                            needs_mapping = False
                            if 'Year' not in save_df.columns or 'Value' not in save_df.columns:
                                needs_mapping = True
                                st.warning("Data doesn't have standard column names (Year, Value). Using original column names.")
                            
                            # If data is validated but doesn't have standard names, try to detect and map
                            if not needs_mapping:
                                # Data already has proper columns, use as is
                                pass
                            else:
                                # Use the data as-is but warn user about potential issues
                                st.info("Note: Data saved with original column names. Consider using 'Update Mapping and Save' if you encounter issues.")
                                
                        except Exception as e:
                            st.warning(f"Could not check column mapping: {e}")
                        
                        # Save to session state
                        data['country_historical'] = save_df
                        st.session_state.country_historical = save_df
                        
                        # Also save to configured file path for persistence
                        try:
                            source_path = config_manager.get_data_source_path('country_historical')
                            import os
                            os.makedirs(os.path.dirname(source_path), exist_ok=True)
                            ...
                            st.success(f"Country historical data saved to session and file: {source_path}")
                ...
                if is_valid:
                    ...
                    if st.button("Add Indicator"):
                        ...
                        try:
                            ...
                            # Create filename from indicator name (sanitize for filesystem)
                            safe_filename = "".join(c for c in indicator_name if c.isalnum() or c in (' ', '-', '_')).rstrip()
                            safe_filename = safe_filename.replace(' ', '_').lower()
                            file_path = os.path.join(indicators_dir, f"{safe_filename}.xlsx")
                            
                            # Save to Excel file
                            df.to_excel(file_path, index=False)
                            
                            # Update configuration to include the new indicator
                            try:
                                indicators_config = config_manager.get_indicators() or []
                                
                                # Check if indicator already exists in config
                                existing_indicator = None
                                for i, ind in enumerate(indicators_config):
                                    if ind.get('name') == indicator_name:
                                        existing_indicator = i
                                        break
                                
                                # Create indicator config entry
                                indicator_config = {
                                    "name": indicator_name,
                                    "type": indicator_type,
                                    "weight": weight_value,
                                ...
                                if existing_indicator is not None:
                                    ...
                                    indicators_config[existing_indicator] = indicator_config
                                else:
                                    # Add new indicator
                                    indicators_config.append(indicator_config)
                                
                                # Update the configuration (Note: This is a simplified approach)
                                # In a full implementation, you'd want to update the actual config file
                                config_manager.config['data_sources']['indicators'] = indicators_config
                                
                                st.success(f"Indicator '{indicator_name}' added successfully and saved to {file_path}!")
                                
                            except Exception as config_error:
                                st.warning(f"Indicator saved but couldn't update configuration: {config_error}")
                                st.success(f"Indicator '{indicator_name}' saved to {file_path}!")
                            ...
                            st.warning(f"Indicator added to session but couldn't save to file: {str(e)}")
