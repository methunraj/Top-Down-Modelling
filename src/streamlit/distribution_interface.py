...
def render_distribution_year_settings(market_distributor: MarketDistributor) -> Dict[str, Any]:
    """
    Render distribution start year settings interface.
    
    Args:
        market_distributor: MarketDistributor instance
        
    Returns:
        Dictionary with updated distribution settings
    """
    st.subheader("🔒 Historical Data Preservation Settings")
    
    st.info("""
    **Start Distribution From Year**: This preserves historical data exactly as-is up to a certain year.
    Only data from the specified year onwards will use the distribution algorithm.
    
    Example: If you set 2023 as the distribution start year:
    - Years 2018-2022: Preserved exactly from your country historical data (no changes)
    - Years 2023-2030: New distribution applied based on global forecast and market dynamics
    """)
    
    # Get distribution settings
    distribution_settings = market_distributor.distribution_settings
    distribution_start_year = distribution_settings.get('distribution_start_year', None)
    
    # Enable distribution from specific year
    enable_distribution_year = st.checkbox(
        "Start Distribution From Specific Year",
        value=distribution_start_year is not None,
        help="Preserve historical data exactly as-is before a specific year"
    )
    
    if enable_distribution_year:
        ...
        # Always use number input to allow any year selection
        # Display available historical years for reference
        if available_years:
            st.info(f"📊 Your historical data covers years: {min(available_years)} to {max(available_years)}")
        
        # Dynamic year range based on available data
        min_year = min(available_years) - 10 if available_years else 2000
        max_year = max(available_years) + 20 if available_years else 2050
        default_year = int(distribution_start_year) if distribution_start_year is not None else (max(available_years) + 1 if available_years else None)
        
        distribution_year = st.number_input(
            "Distribution Start Year",
            min_value=min_year,
            max_value=max_year,
            value=default_year,
            help="Historical data before this year will be preserved exactly as-is. Can be any year including future years."
        )
        
        # Validation and user feedback
        if available_years:
            if distribution_year <= max(available_years):
                preserved_years = [y for y in available_years if y < distribution_year]
                distributed_years = [y for y in available_years if y >= distribution_year]
                st.success(f"✅ **Years {min(preserved_years) if preserved_years else 'None'}-{max(preserved_years) if preserved_years else 'None'}**: Preserved from historical data")
                if distributed_years:
                    st.warning(f"🔄 **Years {min(distributed_years)}-{max(distributed_years)}**: Will be redistributed")
            else:
                st.success(f"✅ **Years {min(available_years)}-{max(available_years)}**: All historical data preserved")
                st.info(f"🔄 **Years {distribution_year}+**: Only forecast years will be distributed")
        
        st.info(f"📝 Distribution will start from year **{distribution_year}** forward.")
        ...
        # Return updated settings
        return {
            'distribution_start_year': distribution_year,
            'enable_regional_aggregation': enable_regional
        }
    else:
        # Add option to disable regional aggregation even when distribution year is off
        st.subheader("Regional Aggregation")
        enable_regional = st.checkbox(
            "Enable Regional Aggregation",
            value=distribution_settings.get('enable_regional_aggregation', True),
...
def render_distribution_interface(market_distributor=None, config_manager=None) -> dict:
    ...
    with tab5:
        distribution_year_settings = render_distribution_year_settings(market_distributor)
        distribution_settings.update(distribution_year_settings)
...
def render_indicator_influence_analysis(indicator_analyzer, distributed_data=None) -> None:
    ...
    try:
        ...
        # Handle both old and new indicator analyzer versions
        try:
            indicator_correlations = indicator_analyzer.get_indicator_correlations()
        except AttributeError:
            # Fallback for enhanced analyzer
            if hasattr(indicator_analyzer, 'correlation_results'):
                indicator_correlations = {
                    indicator: results.get('pearson', 0.0) 
                    for indicator, results in indicator_analyzer.correlation_results.items()
                }
            else:
                indicator_correlations = {}
