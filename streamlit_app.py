...
# Utility function to standardize data columns for visualization
def standardize_value_column(df):
    """Ensure dataframe has a 'Value' column for visualization compatibility"""
    if df is None:
        return None
    ...
    value_columns = ['Value', 'value', 'market_value', 'Market_Value', 'market_size', 'Market_Size']
    
    # Check if we already have 'Value'
    if 'Value' in df_copy.columns:
        return df_copy
    
    # Look for alternative value columns
    for col in value_columns:
        if col in df_copy.columns:
            df_copy = df_copy.rename(columns={col: 'Value'})
            logger.info(f"Renamed column '{col}' to 'Value' for visualization")
            return df_copy
    
    # If no standard column found, look for numeric columns
    numeric_cols = df_copy.select_dtypes(include=[np.number]).columns.tolist()
    exclude_cols = ['Year', 'year', 'idGeo', 'id', 'ID', 'index']
    ...
    if numeric_cols:
        ...
        df_copy = df_copy.rename(columns={numeric_cols[0]: 'Value'})
        logger.info(f"Renamed column '{numeric_cols[0]}' to 'Value' for visualization")
    else:
        logger.warning("No suitable value column found for visualization")
...
def sidebar_navigation():
    ...
    # Add Monte Carlo status if on Market Distribution page
    if st.session_state.active_page == "Market Distribution":
        if hasattr(st.session_state, 'monte_carlo_settings') and st.session_state.monte_carlo_settings.get('enabled', False):
            if st.session_state.monte_carlo_settings.get('enhanced_mode', False):
                data_status["Monte Carlo"] = "🚀 Enhanced Mode"
            else:
                data_status["Monte Carlo"] = "✅ Standard Mode"
        else:
            data_status["Monte Carlo"] = "⭕ Disabled"
...
def render_home_page():
    ...
    if st.session_state.global_forecast is not None:
        st.info("📊 **Current Data Status**: User-uploaded data is loaded")
        # Show year range
        if 'Year' in st.session_state.global_forecast.columns:
            years = sorted(st.session_state.global_forecast['Year'].unique())
            st.write(f"Global forecast years: {min(years)} - {max(years)}")
    else:
        st.warning("📊 **Current Data Status**: No data loaded. Please upload your data or use test data.")
    ...
    with col1:
        if st.button("Load Test Data (with sample indicators)", key="load_test_data_with_indicators"):
            ...
            if (st.session_state.global_forecast is not None or 
                st.session_state.country_historical is not None):
                if not st.checkbox("⚠️ This will OVERWRITE your uploaded data. Continue?", key="confirm_test_data_1"):
                    st.warning("Test data loading cancelled to preserve your uploaded data.")
                    return
    ...
    with col2:
        if st.button("Load Test Data (no indicators)", key="load_test_data_no_indicators"):
            ...
            if (st.session_state.global_forecast is not None or 
                st.session_state.country_historical is not None):
                if not st.checkbox("⚠️ This will OVERWRITE your uploaded data. Continue?", key="confirm_test_data_2"):
                    st.warning("Test data loading cancelled to preserve your uploaded data.")
                    return
    ...
    with col1:
        st.subheader("Enhanced Monte Carlo")
        st.markdown("""
        **NEW!** Enhanced Monte Carlo in Market Distribution:
    ...
    with col2:
        ...
        st.markdown("""
        Enhanced Monte Carlo capabilities:
        ...
        """)
        st.info("💡 **Tip**: Select 'Enhanced Live Monte Carlo' mode in Market Distribution for advanced features")
...
def render_current_page():
    ...
    if 'data_loader' not in st.session_state or st.session_state.data_loader is None:
        st.session_state.data_loader = DataLoader(config_manager)
    ...
    if st.session_state.active_page == "Home":
        ...
        data = render_data_upload(config_manager, data_loader)
        ...
        # Apply column standardization before rendering visualization
        if st.session_state.distributed_market is not None:
            st.session_state.distributed_market = standardize_value_column(st.session_state.distributed_market)
            render_enhanced_visualization_interface(config_manager)
        else:
            st.error("No distributed market data available! Please run market distribution first.")
            if st.button("Go to Market Distribution"):
                st.session_state.active_page = "Market Distribution"
                st.rerun()
