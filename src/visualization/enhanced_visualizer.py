    ...
    def create_animated_bar_race(self, market_data: pd.DataFrame, 
        ...
        for year in years:
            ...
            frame = go.Frame(
                data=[go.Bar(
                    ...
                    text=[format_market_value(val) for val in year_data['Value']],
        ...
        fig = go.Figure(
            data=[go.Bar(
                ...
                text=[format_market_value(val) for val in initial_data['Value']],
                textposition='outside',
                textfont=dict(size=12)
            )],
            frames=frames
    ...
    def create_ranking_table_with_sparklines(self, market_data: pd.DataFrame,
        ...
        for rank, (_, row) in enumerate(ref_data.iterrows(), 1):
            ...
            table_data.append({
                'Rank': rank,
                'Country': country,
                'Value': format_market_value(current_value),
                'CAGR': f"{cagr:.1f}%",
                'Trend': sparkline,
